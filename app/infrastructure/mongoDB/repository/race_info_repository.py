...
class RaceInfoRepository(MongoDBRepository):
    def __init__(self, race_data_repository:IGenericRepository):
        super().__init__('race_info', RaceInfoEntity, RaceInfoModel)
        self.__race_data_repository = race_data_repository

    def get_all_raw(self) -> List[RaceInfoRawModel]:
        all_race_info_models:List[RaceInfoModel] = self.get_all()
        ...
        for race_info_model in all_race_info_models:
            raw_race_info_model:RaceInfoRawModel = RaceInfoRawModel()
            ...
            for raw_race_data_model in all_raw_race_data_model:
                if race_info_model.race_data_id == raw_race_data_model.id:
    ...
    def get_raw_by_id(self, model_id:str) -> Optional[RaceInfoRawModel]:
        race_info_model:RaceInfoModel = self.get_by_id(model_id)

...
class BaseController(IBaseController):
    def __init__(self, base_service:BaseService, model_api_response:BaseAPI_Model, model_domain:BaseModel):
        ...
        self.__base_service = base_service
        self.__model_api_response = model_api_response
        self.__model_domain = model_domain

    def get_all(self):
        try:
            results = self.__base_service.get_all()
            results = [self.__model_api_response().create_by_domain_model(result) for result in results]
    ...
    def get_by_id(self, model_id):
        try:
            result_model_domain:BaseModel = self.__base_service.get_by_id(model_id)
            ...
            return CustomStaticJSONResponse.success(data=self.__model_api_response().create_by_domain_model(result_model_domain))
        except Exception as exception_error:
    ...
    def add(self, new_model:BaseAPI_Model):
        try:
            result_model_domain = self.__base_service.add(new_model.to_domain_model(self.__model_domain))
    ...
    def update_by_id(self, model_id:str, new_model: BaseAPI_Model):
        try:
            result_model_domain = self.__base_service.update_by_id(model_id, new_model.to_domain_model(self.__model_domain))
    ...
    def delete_by_id(self, model_id):
        try:
            status = self.__base_service.delete_by_id(model_id)
